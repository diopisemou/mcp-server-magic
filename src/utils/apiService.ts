...
export async function saveApiDefinition(
  ...
  const definition = {
    ...apiDefinition,
    endpoint_definition: endpointDefinitions,
  };
  const { data, error } = apiDefinition.id
    ? await supabase.from("api_definitions").update(definition).eq(
      "id",
      apiDefinition.id,
    ).select().single()
    : await supabase.from("api_definitions").insert(definition).select()
      .single();
...
export async function getApiDefinition(id: string): Promise<ApiDefinition> {
  const { data, error } = await supabase.from("api_definitions").select("*").eq(
    "id",
    id,
  ).single();
...
export async function getApiDefinitions(): Promise<ApiDefinition[]> {
  const { data, error } = await supabase.from("api_definitions").select("*")
...
export async function deleteApiDefinition(id: string): Promise<boolean> {
  const { error } = await supabase.from("api_definitions").delete().eq(
    "id",
    id,
  );
  if (error) {
    throw new Error(`Failed to delete API definition: ${error.message}`);
  }
  return true;
}
