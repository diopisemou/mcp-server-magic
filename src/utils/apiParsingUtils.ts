...
export function parseApiDefinition(
  apiDefinition: ApiDefinition | null,
): ValidationResult {
  if (!apiDefinition?.content) {
    return {
      endpoints: [],
      isValid: false,
      error: "No content detected",
      format: apiDefinition.format,
      parsedDefinition: apiDefinition,
    } as ValidationResult;
  }

  try {
    return {
      endpoints: extractEndpointsFromDefinition(apiDefinition.content),
      isValid: true,
      error: null,
      format: apiDefinition.format,
      parsedDefinition: apiDefinition,
