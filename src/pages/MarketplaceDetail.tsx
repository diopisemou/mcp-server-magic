...
import { ArrowLeft, Star, Download, Copy, CheckCircle, AlertCircle, Server, Tool } from 'lucide-react';
...
const mockServers: MCPServer[] = [
  {
    ...
    description: 'Access current weather data and forecasts for cities worldwide',
    author: 'WeatherAPI Team',
    version: '1.2.0',
    stars: 245,
    downloads: 5432,
    capabilities: [
      { type: 'resource', name: 'getCurrentWeather', description: 'Get current weather for a location' },
      { type: 'resource', name: 'getForecast', description: 'Get weather forecast for a location' }
    ],
  ...
  },
  {
    ...
    downloads: 3891,
    capabilities: [
      { type: 'resource', name: 'getTopHeadlines', description: 'Get top news headlines' },
      { type: 'resource', name: 'searchNews', description: 'Search for news articles' }
    ],
  ...
  },
  {
    ...
    downloads: 8901,
    capabilities: [
      { type: 'tool', name: 'storeData', description: 'Store data in the database' },
      { type: 'tool', name: 'queryData', description: 'Query data from the database' },
      { type: 'tool', name: 'updateData', description: 'Update existing data' },
      { type: 'tool', name: 'deleteData', description: 'Delete data from the database' }
    ],
  ...
  },
  {
    ...
    downloads: 6789,
    capabilities: [
      { type: 'tool', name: 'translateCode', description: 'Translate code between languages' },
      { type: 'tool', name: 'formatCode', description: 'Format code according to language standards' }
    ],
...
export default function MarketplaceDetail() {
  ...
  const handleCopyToClipboard = () => {
    const capabilitiesText = server.capabilities
      .map(cap => `  - ${cap.type === 'resource' ? 'Resource' : 'Tool'}: ${cap.name} - ${cap.description}`)
      .join('\n');
      ...
      <div className="container mx-auto py-10">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-6 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mb-4">by {server.author} • v{server.version}</p>
          ...
          <TabsContent value="capabilities" className="pt-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>This MCP server provides the following capabilities to AI models</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="integration" className="pt-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>How to integrate this MCP server with AI models</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-2">
                      You can use this MCP server with Claude by adding it as a context source:
                    </p>
...
"Please use the ${server.name} to [perform specific action]"
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      If you want to integrate this MCP server with your own applications, 
                      you can use the following API endpoint:
                    </p>
                    <div className="bg-muted p-4 rounded-md mt-2">
                      <code className="text-xs">https://api.mcpmarketplace.com/servers/{server.id}</code>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 bg-blue-50 p-4 rounded-md">
                    ...
                    <p className="text-sm text-blue-700">
                      ...
                      <a href="/docs" className="text-blue-600 underline ml-1">Documentation</a> 
